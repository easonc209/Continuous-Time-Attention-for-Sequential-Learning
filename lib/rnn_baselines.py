...
class Classic_RNN(Baseline):
    def __init__(self, input_dim, latent_dim, device, 
        ...
        if concat_mask:
            encoder_dim = encoder_dim +1 # + 1 for meld, * 2 for others
    ...
    def get_reconstruction(self, time_steps_to_predict, data, truth_time_steps, 
        ...
        outputs = all_hiddens
        if self.att:
            outputs, attScore = utils.run_attention(all_hiddens)
            extra_info["attScore"] = attScore
        ...
        if self.use_binary_classif:
            if self.classif_per_tp:
                extra_info["label_predictions"] = self.classifier(outputs)
            else:
