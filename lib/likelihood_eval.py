...
def compute_binary_CE_loss(label_predictions, mortality_label):
    ...
    idx_not_nan = (1 - torch.isnan(mortality_label).float()).nonzero()
...
def compute_multiclass_CE_loss(label_predictions, true_label, mask):
	...
	true_label = true_label.reshape(n_traj_samples * n_traj * n_tp, n_dims) # 1 for meld, n_dims for others
...
def compute_mse(mu, data, mask = None):
    ...
    if mask is None:
        ...
        data_masked = torch.masked_select(data, mask.bool())
        mu_masked = torch.masked_select(mu, mask.bool())
        res = mse(mu_masked, data_masked)
    return res
