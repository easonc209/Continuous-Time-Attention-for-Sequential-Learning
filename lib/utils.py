...
# create nn
def create_net_with_time(n_inputs, n_outputs, n_layers = 1, 
    n_units = 100, layer_type = "concat",nonlinear = "tanh"):
...
def reverse_dim1(tensor):
    idx = [i for i in range(tensor.size(1)-1, -1, -1)]
    return tensor[:,idx]


def reverse_dim2(tensor):
...
def run_attention(all_hiddens):
    n_traj_samples, n_traj, n_tp, n_dims = all_hiddens.size()
    mask = torch.tril(torch.ones(n_tp, n_tp)).to(all_hiddens)
    all_hiddens = all_hiddens.reshape(n_traj_samples*n_traj, n_tp, n_dims)
    
    attScore = torch.tanh(torch.squeeze(torch.bmm(all_hiddens, all_hiddens.transpose(1,2)))) 
    # (n_traj, n_tp, n_dims)  X (n_traj, n_dims,  n_tp) == (n_traj, n_tp,  n_tp)
    attScore = F.dropout(F.softmax(attScore*mask - 1e3*(1-mask), dim=-1))
    # (n_traj, n_tp, n_tp)
    context = torch.squeeze(torch.bmm(attScore, all_hiddens))
    # (n_traj, n_tp, n_tp)  X (n_traj, n_tp, n_dims) == (n_traj, n_tp, n_dims)
    output = (all_hiddens+context).unsqueeze(0)
    return output, attScore
...
def compute_loss_all_batches(model,
    ...
    for i in range(n_batches):
        ...
        if args.classif:
            ...
            all_test_labels = torch.cat((all_test_labels, 
                batch_dict["labels"].reshape(-1, n_labels)),0) # 1 for meld, n_labels for others
    ...
    if args.classif:
        ...
        if args.dataset == "meld":
            ...
            labeled_tp = torch.sum(all_test_labels, -1) < n_labels # Not one-hot

            all_test_labels = all_test_labels[labeled_tp]
            classif_predictions = classif_predictions[:, labeled_tp]

            # classif_predictions is in on-hot-encoding -- convert to class ids
            _, pred_class_id = torch.max(classif_predictions, -1)
            ...
            total["accuracy"] = sk.metrics.accuracy_score(
                    ...
                    pred_class_id.cpu().numpy())
            total["classification_report"] = sk.metrics.classification_report(
                    all_test_labels.cpu().numpy(), 
                    pred_class_id.cpu().numpy(),
